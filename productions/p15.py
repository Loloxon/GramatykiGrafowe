...
class P15(Production):
    ...
    def transform(
        ...
        node_center = graph.create_center_node(
            [node_mapper(6), node_mapper(7), node_mapper(8), node_mapper(9), node_mapper(10)]
        )

        graph.add_edge(node_mapper(6), node_center)
        graph.add_edge(node_mapper(7), node_center)
        graph.add_edge(node_mapper(8), node_center)
        graph.add_edge(node_mapper(9), node_center)
        graph.add_edge(node_mapper(10), node_center)

        graph.add_hyperedge([
            node_mapper(1), node_mapper(6), node_center, node_mapper(7)
        ])
        graph.add_hyperedge([
            node_mapper(6), node_mapper(2), node_mapper(10), node_center
        ])
        graph.add_hyperedge([
            node_mapper(10), node_mapper(5), node_mapper(9), node_center
        ])
        graph.add_hyperedge([
            node_mapper(9), node_mapper(3), node_mapper(8), node_center
        ])
        graph.add_hyperedge([
        ...
        node_mapper(9).is_hanging = False
        node_mapper(10).is_hanging = False
